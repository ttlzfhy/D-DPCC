...
def test_one_frame(x, ckptdir_path, out_path, scaling_factor=1.0, rho=1.0, res=1024):
    # load data
    start_time = time.time()
    # x = sort_spare_tensor(input_data)
    ...
    bits = np.array([os.path.getsize(filename + postfix) * 8 \
                     for postfix in ['_C.bin', '_F.bin', '_H.bin', '_num_points.bin']])
    bpps = (bits / len(x)).round(3)
    bpp = bpps[0] + bpps[1] + bpps[2] + bpps[3]
    ...
    start_time = time.time()
    write_ply_open3d_normal(filename+'_ori.ply', x.C.detach().cpu().numpy()[:, 1:])
    pc_error_metrics = pc_error(filename+'_ori.ply', filename + '_dec.ply',
                                res=res, normal=True, show=False)
    d1psnr = pc_error_metrics["mseF,PSNR (p2point)"][0]
    d2psnr = pc_error_metrics["mseF,PSNR (p2plane)"][0]
